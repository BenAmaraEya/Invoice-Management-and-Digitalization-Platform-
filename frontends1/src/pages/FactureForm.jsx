...
function FactureForm() {
  ...
  const fetchFournisseurDetails = async () => {
    try {
      ...
      // Effectuer une seule requête pour récupérer toutes les informations nécessaires
      const [fournisseurResponse, userResponse] = await Promise.all([
        axios.get(`http://localhost:3006/fournisseur/userId/${id}`),
        axios.get(`http://localhost:3006/user/${id}`)
      ]);
  
      const fournisseurIdFiscale = fournisseurResponse.data.fournisseur.idfiscale;
      const fournisseurName = userResponse.data.name;
  
      // Vérifier que les données nécessaires sont présentes dans la réponse
      if (fournisseurIdFiscale && fournisseurName) {
        setFormData(prevData => ({
          ...prevData,
          idfiscale: fournisseurIdFiscale,
        ...
        console.error('Fournisseur idfiscale or name not found in the response');
      }
    } catch (error) {
...
function FactureForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const result = await axios.get(`http://localhost:3006/fournisseur/userId/${id}`);
      const iderp = result.data.fournisseur.iderp;
      ...
      console.log('données de facture ajouté avec succès.');
      alert('Facture data added successfully.');
  ...
  return (
    <section>
      <Container className="facture-container d-flex justify-content-between">
        <Row>
          <p className='form-paragraph' style={{textAlign:'center'}}>
            Vérifiez et complétez votre formulaire attentivement avant de soumettre.
          </p>
          <div>
            <div className="facture-form">
              <Form onSubmit={handleSubmit}>
                <Row>
                  ...
                  <Col md="6">
                    ...
                    <FormGroup>
                      ...
                      <select className="facture-input" name="objet" value={formData.objet} onChange={handleChange} required>
                        <option value="NOUVELLE FACTURE">Nouvelle Facture</option>
                        <option value="ANCIENNE FACTURE">Ancienne Facture</option>
                      </select>
                    </FormGroup>
                    <FormGroup>
                      <label className="facture-label">Pièces Jointes :*</label>
                      <div>
                        <label>
                          <input
                            ...
                            checked={formData.piece_name.includes('bon de commande')}
                            onChange={handleChange}
                          />
                          Bon de Commande
                        </label>
                    ...
                    <FormGroup>
                      <label className="facture-label">Délai de Paiement:*</label>
                      <input className="facture-input" type="text" name="delai_paiement" value={formData.delai_paiement} onChange={handleChange} disabled />
                    </FormGroup>
                  </Col>
                </Row>
                <div className='btn-container' style={{ marginLeft: '40%' }}>
