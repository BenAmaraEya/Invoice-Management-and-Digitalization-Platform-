...
const UpdateFacture = () => {
  ...
  useEffect(() => {
    const fetchFactureDetails = async () => {
      try {
        ...
        const response = await axios.get(`http://${AdresseIp}:3006/facture/facturebyId/${idF}`, {
          headers: {
  ...
  const changeDocuments = async (event) => {
    ...
    try {
      ...
      const token = await AsyncStorage.getItem('accessToken');
      const response = await axios.post(`http://${AdresseIp}:3006/facture/upload`, formData, {
        headers: {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Update facture
      const updateFactureResponse = await axios.put(`http://${AdresseIp}:3006/facture/updateFacture/${idF}`, {
        ...formData,
      }, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      const updatePiecesResponse = await axios.put(`http://${AdresseIp}:3006/piecejoint/updatepiece/${idF}`, {
        piece_name: selectedPieces, 
      }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
